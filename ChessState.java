...
class ChessState {
	...
	static class ChessMoveIterator
	{
        ...
        int minimax(int[] inBoard, int depth, int alpha, int beta, boolean maxPlayer) {
            
            int[] node = inBoard.clone();
            int score;
            
            if(depth == 0 ){ // || isWinner(node)) {
...
//                return score;
            ...
            if(maxPlayer) {
                ...
                for(int i = 0; i < node.length; i++) {
                    if(node[i] == 0) {
                        node[i] = 2;
                        score = Math.max(score, minimax(node,depth-1,alpha,beta,false));
                ...
                for(int i = 0; i < node.length; i++) {
                    if(node[i] == 0) {
                        node[i] = 1;
                        score = Math.min(score, minimax(node,depth-1,alpha,beta,true));
